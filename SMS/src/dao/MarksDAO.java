...
public class MarksDAO {
    ...
    public List<Map<String,Object>> getStudentSummary() {
        ...
        String sql = "SELECT s.id, s.name, s.rollNumber, s.course, " +
                     ...
                     "ROUND(CAST(AVG(m.marks * 100.0 / m.maxMarks) AS NUMERIC), 1) avgPct " +
    ...
    // Add or update marks — PostgreSQL ON CONFLICT replaces MySQL ON DUPLICATE KEY
    public boolean saveMarks(int studentId, int subjectId,
        ...
        String sql = "INSERT INTO marks (studentId, subjectId, marks, maxMarks, examType, grade) " +
                     "VALUES (?,?,?,?,?,?) " +
                     "ON CONFLICT ON CONSTRAINT uq_marks DO UPDATE SET " +
                     "marks=EXCLUDED.marks, maxMarks=EXCLUDED.maxMarks, grade=EXCLUDED.grade";
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
